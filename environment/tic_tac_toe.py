...
class TicTacToeEnv(gym.Env):
    def __init__(self, rows_cols=3):
        self.rows_cols = rows_cols  # 井字棋的行列数
        ...
        for i in range(self.rows_cols):
            for j in range(self.rows_cols):
                if self.state[i][j] == 1:
                    ...
                    move = rendering.Transform(translation=(
                    i * self.line_size + self.line_size / 2, j * self.line_size + self.line_size / 2))  # 创建平移操作
                ...
                if self.state[i][j] == -1:
                    ...
                    move = rendering.Transform(
                        translation=(i * self.line_size + self.line_size / 2, j * self.line_size + self.line_size / 2))
